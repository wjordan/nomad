...
// linuxNetworkInterfaceDetector implements a Linux-optimized interface detector.
type linuxNetworkInterfaceDetector struct {
	interfaces map[int]netInterface
}

// netInterface associates a network interface with its set of IP addresses.
type netInterface struct {
	net.Interface
	addrs *[]net.Addr
}

// Interfaces uses Linux-optimized sockaddr.GetAllInterfaces() to fetch all interfaces and associated IP addresses.
func (b *linuxNetworkInterfaceDetector) Interfaces() ([]net.Interface, error) {
	interfaces := make([]net.Interface, 0)
	ifMap := make(map[int]netInterface, 0)
	b.interfaces = ifMap
	...
	for _, ifAddr := range ifAddrs {
		ipNet := (*sockaddr.ToIPAddr(ifAddr.SockAddr)).NetIPNet()
		if ifi, ok := ifMap[ifAddr.Interface.Index]; ok {
			*ifi.addrs = append(*ifi.addrs, ipNet)
		} else {
			ifMap[ifAddr.Interface.Index] = netInterface{ifAddr.Interface, &[]net.Addr{ipNet}}
			interfaces = append(interfaces, ifAddr.Interface)
		}
	}
